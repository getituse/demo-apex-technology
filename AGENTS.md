...
- **GitHub Actions Pages Deployment Workflow complete (2026-09-14):** Added production-ready GitHub Actions deployment workflow (`.github/workflows/deploy.yml`) for GitHub Pages with automatic site URL resolution, `.nojekyll` bypass, and static artifact publishing (`dist/`). Updated `src/site/config.ts` to support `process.env.SITE_URL` so subpath deployments (e.g. `https://getituse.github.io/demo-apex-technology`) dynamically set the Vite base and React Router basename while retaining localhost/default fallback for local development. Full static build verified cleanly (66 routes prerendered).
